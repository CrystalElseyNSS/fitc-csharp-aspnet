...
const PoolRequest_1 = require("./PoolRequest");
...
class Pool extends events_1.EventEmitter {
    constructor(factory, config) {
        ...
        this._state = definitions_1.PoolState.IDLE;
        ...
        const opts = this._options = new PoolOptions_1.PoolOptions(this);
     ...
     */
    start() {
        if (this._state === definitions_1.PoolState.STARTED)
            return;
        if (this._state >= definitions_1.PoolState.CLOSING)
            throw new Error('Closed pool can not be started again');
        this._state = definitions_1.PoolState.STARTED;
    ...
    close(arg0, arg1) {
        ...
        if (this._state === definitions_1.PoolState.CLOSED || this._state === definitions_1.PoolState.IDLE)
            return callback();
        if (this._state === definitions_1.PoolState.CLOSING) {
        ...
        this._state = definitions_1.PoolState.CLOSING;
    ...
    acquire(arg0) {
        ...
        this._requestQueue.push(new PoolRequest_1.PoolRequest(this, callback));
    ...
    release(resource, callback) {
        ...
        if (item && item.state !== definitions_1.ResourceState.IDLE)
    ...
    _createResource(request, callback) {
        ...
        const handleCallback = (err, obj) => {
            ...
            if (err || !obj) {
                ...
                if (err instanceof AbortError_1.AbortError || tries >= maxRetries) {
                    this._creating--;
            ...
            const item = new ResourceItem_1.ResourceItem(obj);
        ...
        const tryCreate = () => {
            try {
                ...
                if (!o)
                    return handleCallback(new AbortError_1.AbortError('Factory returned no resource'));
    ...
    _setHouseKeep(ms) {
        ...
        if (ms > 0 && this.state === definitions_1.PoolState.STARTED || this.state === definitions_1.PoolState.CLOSING)
            this._houseKeepTimer = setInterval(() => this._houseKeep(), ms);
    }
    _houseKeep() {
        const isClosing = this._state === definitions_1.PoolState.CLOSING;
        const now = Date.now();
        ...
        if (isClosing) {
            ...
            if (this._allResources.size)
                return;
            clearInterval(this._houseKeepTimer);
            this._state = definitions_1.PoolState.CLOSED;
            this._requestsProcessing = 0;
            this.emit('close');
        }
    }
    ...
    _itemSetAcquired(item) {
        if (item.state !== definitions_1.ResourceState.ACQUIRED) {
            this._itemDetach(item);
            item.state = definitions_1.ResourceState.ACQUIRED;
    ...
    _itemDetach(item) {
        switch (item.state) {
            case definitions_1.ResourceState.IDLE:
                item.idleTime = 0;
                /* istanbul ignore next*/
                if (item.idleNode)
                ...
                break;
            case definitions_1.ResourceState.ACQUIRED:
            case definitions_1.ResourceState.VALIDATION:
                /* istanbul ignore next*/
    ...
    _itemSetIdle(item, callback) {
        const isAcquired = item.state === definitions_1.ResourceState.ACQUIRED;
        const handleCallback = (err) => {
            if (err)
                return this._itemDestroy(item, callback);
            this._itemDetach(item);
            item.idleTime = Date.now();
            item.state = definitions_1.ResourceState.IDLE;
    ...
    _itemValidate(item, callback) {
        item.state = definitions_1.ResourceState.VALIDATION;
        try {
