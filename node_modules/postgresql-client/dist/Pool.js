...
class Pool extends SafeEventEmitter_1.SafeEventEmitter {
    constructor(config) {
        super();
        const cfg = connection_config_1.getConnectionConfig(config);
        this.config = Object.freeze(cfg);
        const poolOptions = {};
        poolOptions.acquireMaxRetries = putil_varhelpers_1.coerceToInt(cfg.acquireMaxRetries, 0);
        poolOptions.acquireRetryWait = putil_varhelpers_1.coerceToInt(cfg.acquireRetryWait, 2000);
        poolOptions.acquireTimeoutMillis = putil_varhelpers_1.coerceToInt(cfg.acquireTimeoutMillis, 0);
        poolOptions.idleTimeoutMillis = putil_varhelpers_1.coerceToInt(cfg.idleTimeoutMillis, 30000);
        poolOptions.max = putil_varhelpers_1.coerceToInt(cfg.max, 10);
        poolOptions.maxQueue = putil_varhelpers_1.coerceToInt(cfg.maxQueue, 1000);
        poolOptions.max = putil_varhelpers_1.coerceToInt(cfg.max, 10);
        poolOptions.min = putil_varhelpers_1.coerceToInt(cfg.min, 0);
        poolOptions.minIdle = putil_varhelpers_1.coerceToInt(cfg.minIdle, 0);
        poolOptions.validation = putil_varhelpers_1.coerceToBoolean(cfg.validation, false);
        const poolFactory = {
            create: async () => {
    ...
    async prepare(sql, options) {
        ...
        statement.once('close', () => this._pool.release(common_1.getIntlConnection(connection)));
        return statement;
    }
    release(connection) {
        return this._pool.releaseAsync(common_1.getIntlConnection(connection));
    }
}
exports.Pool = Pool;
