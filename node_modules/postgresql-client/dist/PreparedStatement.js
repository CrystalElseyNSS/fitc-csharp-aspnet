...
class PreparedStatement extends SafeEventEmitter_1.SafeEventEmitter {
    ...
    static async prepare(connection, sql, options) {
        const intoCon = common_1.getIntlConnection(connection);
    ...
    async execute(options = {}) {
        ...
        const autoCommit = putil_varhelpers_1.coerceToBoolean(options.autoCommit != null ?
            options.autoCommit : intoCon.config.autoCommit, true);
        if (!autoCommit && !transactionCommand)
            await intoCon.startTransaction();
        const result = await intoCon.statementQueue.enqueue(() => this._execute(options));
        if (autoCommit && !transactionCommand)
    ...
    async _execute(options = {}) {
        ...
        try {
            ...
            let resultFields;
            if (fields) {
                parsers = common_1.getParsers(typeMap, fields);
            ...
            if (resultFields && parsers && executeResult.rows) {
                ...
                for (let i = 0; i < l; i++) {
                    row = rows[i];
                    common_1.parseRow(parsers, row, options);
                    if (options.objectRows) {
                        rows[i] = common_1.convertRowToObject(resultFields, row);
                    }
                }
            }
