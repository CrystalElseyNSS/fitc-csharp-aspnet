...
class Backend {
    ...
    parse(data, callback) {
        ...
        const io = new BufferReader_1.BufferReader(data);
        let offsetBookmark;
...
const MessageParsers = {
    [protocol_1.Protocol.BackendMessageCode.Authentication]: parseAuthentication,
    [protocol_1.Protocol.BackendMessageCode.BackendKeyData]: parseBackendKeyData,
    [protocol_1.Protocol.BackendMessageCode.CommandComplete]: parseCommandComplete,
    [protocol_1.Protocol.BackendMessageCode.CopyData]: parseCopyData,
    [protocol_1.Protocol.BackendMessageCode.CopyInResponse]: parseCopyResponse,
    [protocol_1.Protocol.BackendMessageCode.CopyOutResponse]: parseCopyResponse,
    [protocol_1.Protocol.BackendMessageCode.CopyBothResponse]: parseCopyResponse,
    [protocol_1.Protocol.BackendMessageCode.DataRow]: parseDataRow,
    [protocol_1.Protocol.BackendMessageCode.ErrorResponse]: parseErrorResponse,
    [protocol_1.Protocol.BackendMessageCode.NoticeResponse]: parseErrorResponse,
    [protocol_1.Protocol.BackendMessageCode.FunctionCallResponse]: parseFunctionCallResponse,
    [protocol_1.Protocol.BackendMessageCode.NegotiateProtocolVersion]: parseNegotiateProtocolVersion,
    [protocol_1.Protocol.BackendMessageCode.ParameterDescription]: parseParameterDescription,
    [protocol_1.Protocol.BackendMessageCode.ParameterStatus]: parseParameterStatus,
    [protocol_1.Protocol.BackendMessageCode.ReadyForQuery]: parseReadyForQuery,
    [protocol_1.Protocol.BackendMessageCode.RowDescription]: parseRowDescription
};
...
function parseCopyResponse(io) {
    const out = {
        overallFormat: io.readUInt8() === 0 ? protocol_1.Protocol.DataFormat.text : protocol_1.Protocol.DataFormat.binary,
        columnCount: io.readUInt16BE()
    };
    if (out.columnCount) {
        out.columnFormats = [];
        for (let i = 0; i < out.columnCount; i++) {
            out.columnFormats.push(io.readUInt16BE() === 0 ? protocol_1.Protocol.DataFormat.text : protocol_1.Protocol.DataFormat.binary);
        }
    }
    return out;
}
...
function parseRowDescription(io) {
    ...
    for (let i = 0; i < fieldCount; i++) {
        const field = {
            ...
            format: io.readInt16BE() === 0 ? protocol_1.Protocol.DataFormat.text : protocol_1.Protocol.DataFormat.binary
        };
