...
function getConnectionConfig(config) {
    const cfg = config_from_env_1.configFromEnv();
    if (typeof config === 'string') {
        putil_merge_1.default(cfg, parseConnectionString(config));
    }
    else if (typeof config === 'object') {
        putil_merge_1.default(cfg, config);
    }
    if (cfg.host) {
        const x = parseConnectionString('' + cfg.host);
        putil_merge_1.default(cfg, x);
    }
